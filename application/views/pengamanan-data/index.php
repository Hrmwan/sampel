...
function steg_hide($maskfile, $hidefile)
{
	...
	for($i=0,$x=0; $i<sizeof($make_odd) ; $i+=3,$x++)
	{
		...
		{
			if (isset($make_odd[$i+$j]) && $make_odd[$i+$j]===true && is_even($cols[$j]))			{
				// is even, should be odd
				$cols[$j]++;
			} if (isset($make_odd[$i+$j]) && $make_odd[$i+$j]===false && !is_even($cols[$j])){
...
<h3 align=center>IMPLEMENTASI PENGAMANAN DATA BERBASIS WEB <br>DENGAN ALGORITMA
RC4 DAN STEGANOGRAFI LEAST SIGNIFICANT BIT (LSB)<br> PADA
PT.AKSAHARA</h3>
</div>
